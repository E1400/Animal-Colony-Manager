...
2. **Core schema + seed** — rooms/racks/positions, cages, animals, identifiers, placement event logs, husbandry events, real migrations with constraints/indices; a seed script generating a realistic colony (few hundred animals, multiple racks, breeding pairs/litters in flight). **In progress** — Prisma + `@prisma/client` install was interrupted mid-run; `node_modules` is populated but `package-lock.json` didn't regenerate. Run `npm install` once to finish, then `npx prisma init` (already run once — check if `prisma/schema.prisma` exists before re-running) and write the schema per the data model above.
...
- **Neon Postgres project** — not yet created. Needs a free Neon account + project, and the connection string as `DATABASE_URL` in `.env` (not committed).
