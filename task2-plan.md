...
2. **Core schema + seed** — rooms/racks/positions, cages, animals, identifiers, placement event logs, husbandry events, real migrations with constraints/indices; a seed script generating a realistic colony (few hundred animals, multiple racks, breeding pairs/litters in flight). **Schema done, seed not started.** All 21 tables + 15 enums are in `prisma/schema.prisma`; two migrations (`_init`, `_integrity_constraints`) apply cleanly and are verified against a live Postgres. Exclusion constraints over `tstzrange` enforce non-overlapping placements at the DB level — tested by attempting a double-placement and confirming Postgres refuses it. **Next: the seed script** (`prisma/seed.ts`, wired via `prisma7.config.ts`), then the derived-state query layer in `src/lib/queries/` (current placement, as-of placement, animal status from events) with Vitest tests against those queries — that's where a colony manager actually breaks.
3. **Cage & animal views** — current-state derived views, digital cage card (view + print-friendly), QR code generation, mobile-first layout end to end. **Not started.**
4. **QR scan + mobile pass** — camera scan → cage record, one/two-tap common actions, offline read-cache + write-queue decision implemented, real iOS Safari test pass. **Not started.**
...
7. **Compliance + ship** — backup/restore demonstrated, accessibility pass (keyboard nav, contrast, 200% zoom, labels), README (setup, schema rationale + what wasn't normalized, data/cost section), `AI_NOTES.md` finalized, cold-clone verification, 2-3 min demo video shot on an actual phone for the mobile parts. **Not started.**
